...
std::string Base64::encode(const std::vector<byte>& input)
{
    std::string encoded;
...
std::string Base64::encode(const std::string & input) {
    std::vector<byte> data(begin(input), end(input));
    std::string encoded = encode(data);
    return encoded;
}


std::vector<byte> Base64::decode(const std::string& input)
{
    if(input.length() % 4)
...
std::string Base64::decodeToString(const std::string& input) {
    std::vector<byte> bytes = decode(input);
